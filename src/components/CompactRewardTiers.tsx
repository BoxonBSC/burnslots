    ...
    <div className="h-full flex flex-col rounded-2xl bg-gradient-to-b from-muted/40 to-muted/20 border border-border/50 p-3 lg:p-4 backdrop-blur-sm">
      ...
      <div className="mt-auto space-y-1.5 lg:space-y-2">
        ...
        <Collapsible className="mt-2">
          <CollapsibleTrigger className="w-full flex items-center justify-between p-2 lg:p-2.5 rounded-xl bg-gradient-to-r from-neon-orange/10 to-neon-yellow/5 border border-neon-orange/20 hover:border-neon-orange/40 transition-all">
            <span className="text-xs font-display text-neon-orange flex items-center gap-1.5">
              📜 智能合约地址
            </span>
            <ChevronDown className="w-4 h-4 text-neon-orange/60" />
          </CollapsibleTrigger>
          <CollapsibleContent className="mt-2">
            <div className="space-y-2 p-2 lg:p-3 bg-muted/10 rounded-xl border border-border/20">
              {/* 游戏合约 */}
              <div className="space-y-1">
                ...
                <div 
                  className="flex items-center gap-2 p-1.5 bg-muted/30 rounded-lg cursor-pointer hover:bg-muted/50 transition-colors group"
                  onClick={() => {
                    navigator.clipboard.writeText(CYBER_SLOTS_ADDRESS.mainnet);
                    toast.success('游戏合约地址已复制');
                  }}
                >
                  <code className="text-xs text-foreground/80 font-mono truncate flex-1">
                    {CYBER_SLOTS_ADDRESS.mainnet === '0x0000000000000000000000000000000000000000' 
                      ? '待部署...' 
                      : `${CYBER_SLOTS_ADDRESS.mainnet.slice(0, 8)}...${CYBER_SLOTS_ADDRESS.mainnet.slice(-6)}`
                    }
                  </code>
                  <Copy className="w-3 h-3 text-muted-foreground group-hover:text-neon-cyan transition-colors" />
                </div>
              </div>
              ...
              <div className="space-y-1">
                <div className="flex items-center justify-between">
                  <span className="text-xs text-neon-yellow font-display">🪙 代币合约 (CST)</span>
                  <a 
                    href={`https://bscscan.com/token/${CYBER_TOKEN_ADDRESS.mainnet}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-xs text-muted-foreground hover:text-neon-yellow transition-colors flex items-center gap-1"
                  >
                    <ExternalLink className="w-3 h-3" />
                  </a>
                </div>
                ...
                >
                  <code className="text-xs text-foreground/80 font-mono truncate flex-1">
                    {CYBER_TOKEN_ADDRESS.mainnet === '0x0000000000000000000000000000000000000000' 
                      ? '待部署...' 
                      : `${CYBER_TOKEN_ADDRESS.mainnet.slice(0, 8)}...${CYBER_TOKEN_ADDRESS.mainnet.slice(-6)}`
                    }
                  </code>
                  <Copy className="w-3 h-3 text-muted-foreground group-hover:text-neon-yellow transition-colors" />
                </div>
              </div>
